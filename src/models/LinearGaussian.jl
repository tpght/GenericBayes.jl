...
function covariance(model::LinearGaussian, ::GaussianParam)
    Q = inv(model.Σ)
    Q_p = inv(model.Σ_p)
    n = sample_size(model)
    n_p = model.n_p
...
function sample(model::LinearGaussian, θ::GaussianParam, nsamples)
    samples = rand(MvNormal(Vector(map(model, θ)), covariance(model, θ)), nsamples)
    GaussianParam(samples)
end
